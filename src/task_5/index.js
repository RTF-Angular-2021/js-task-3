...
Organaizer.prototype.addMeeting = function(meet){
	if(this.vacations.filter(value => value.isDateInVacation(meet.meetingDate)).length > 0 || this.meetings.filter(value => value.isMeetingInTimeRange(meet.startTime, meet.endTime)
	&& Number(value.meetingDate) == Number(meet.meetingDate)).length > 0){
		return false;
	}
...
Organaizer.prototype.addVacation = function(vacation){
	if(this.vacations.filter(value => value.isDateInVacation(vacation.vacationStartDate) || value.isDateInVacation(vacation.vacationEndDate)).length > 0 
	|| this.meetings.filter(value => vacation.isDateInVacation(value.meetingDate)).length > 0){
		return false;
	} 
